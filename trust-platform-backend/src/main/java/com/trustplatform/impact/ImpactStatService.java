...
public class ImpactStatService {
    ...
    @Cacheable(value = "impact-stats", key = "'all'")
    @Transactional(readOnly = true)
    public List<ImpactStatResponse> getAllStats() {
        log.debug("Cache miss — loading impact stats from DB");
        return impactStatRepository.findAll().stream()
                .map(this::toResponse)
                .collect(Collectors.toList());
    }

    @CacheEvict(value = "impact-stats", allEntries = true)
    @Transactional
    public ImpactStatResponse update(Long id, Long newValue) {
        ImpactStat stat = impactStatRepository.findById(id)
                .orElseThrow(() ->
                        new ResourceNotFoundException("Stat not found: " + id));
        log.info("Impact stat updated: id={}, category={}, old={}, new={}",
